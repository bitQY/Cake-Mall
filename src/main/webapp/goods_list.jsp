    ...
    <div class="container">
        ...
        <c:if test="${not empty goodsPage.total}">
            <div style="text-align:center;">
                <a class="btn btn-info" href="${ctx}/goods_list?&typeId=${type.id}&page=1">首页</a>
                <a class="btn btn-info"${page==1?'disabled':''} href="${ctx}/goods_list?typeId=${type.id}&page=${goodsPage.current-1}">上一页</a>
                <h3 style="display:inline;">[${goodsPage.current}/${goodsPage.pages}]</h3>
                <h3 style="display:inline;">[${goodsPage.total}]</h3>
                <a class="btn btn-info" ${page==goodsPage.pages?'disabled':''}href="${ctx}/goods_list?typeId=${type.id}&page=${goodsPage.current+1}">下一页</a>
                <a class="btn btn-info" href="${ctx}/goods_list?typeId=${type.id}&page=${goodsPage.pages}">尾页</a>
                <form action="${ctx}/goods_list?typeId=${type.id}" method="post">
